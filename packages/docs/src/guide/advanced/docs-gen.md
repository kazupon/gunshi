...
- **No Execution**: Only generates usage text without running command logic. The usage is not asked for through an option, so a command may name its arguments whatever it likes and still be documented: one that declares an argument of its own named `help` or `version` gets a usage all the same, and that usage lists the argument the command declares rather than the global option it replaces
