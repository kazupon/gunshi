...
- **No Execution**: Only generates usage text without running command logic, with one exception. `generate` asks for the usage by passing `-h`, and a command that declares an argument of its own named `help` replaces the global schema whole, so `-h` belongs to that command when it declares `short: 'h'`, and to nothing at all when it does not. Either way no usage is produced: `generate` runs the command and hands back whatever it returns, which is an empty string unless the command returns a string of its own. A schema identical to the global one is indistinguishable from it and keeps the built-in behaviour, so the way to avoid the whole thing is to keep the name out of the commands you generate documentation for
